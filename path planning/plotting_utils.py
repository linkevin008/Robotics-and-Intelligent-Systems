...
def draw_plan(world, plan, bgr=(255,0,0), thickness=1, show_live=False,filename=None):
    img = np.copy(world)
    cv2.circle(img, (int(plan[0].x), int(plan[0].y)), 3, (0,255,0), 3)
    ...
    cv2.circle(img, (int(plan[-1].x), int(plan[-1].y)), 3, (0,0,255), 3)
