...
class RRTPlanner(object):
    ...
    def steer_towards(self, s_nearest, s_rand, max_radius):
        ...
        x = 0
        y = 0
        if (abs(s_nearest.x - s_rand.x) < max_radius and abs(s_nearest.y - s_rand.y) < max_radius):
            s_new = State(s_rand.x, s_rand.y, s_nearest)
            return s_new
        else:
            if (s_rand.y - s_nearest.y == 0):
                angle = 1/cos((s_rand.x-s_nearest.x)/max_radius)
            elif (s_rand,x - s_nearest.x == 0):
                angle = 1/sin((s_rand.y-s_nearest.y)/max_radius)
            else:
                angle = 1/tan(s_rand.y/s_rand.x)
            y = (max_radius * sin(angle)) + s_nearest.y
            x = (max_radius * cos(angle)) + s_nearest.x
    ...
    def path_is_obstacle_free(self, s_from, s_to):
        ...
        for i in range(max_checks):
            ...
            x = (float(i)/max_checks * (s_to.x - s_from.x)) + s_from.x
            y = (float(i)/max_checks * (s_to.y - s_from.y)) + s_from.y
            tempState = State(int(x),int(y),parentState)
            parentState = tempState

            if not (self.state_is_free(tempState)):
                return False
