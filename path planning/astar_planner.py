    ...
    def plan(self, start_state, dest_state):
        ...
        while Q:
            ...
            for ns in self.get_neighboring_states(s):
                ...
                #my code 
                h = sqrt((dest_state.x - ns.x)**2 + (dest_state.y - ns.y)**2)
                g = sqrt((dest_state.x - s.x)**2 + (dest_state.y - s.y)**2)
                ...
                if ((ns not in Q) or (alternative_best_dist_ns < best_dist_found[ns.x, ns.y])) and (h < g):
